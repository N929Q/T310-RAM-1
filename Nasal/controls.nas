...
controls.flapsDown = func(step) {
    ...
    if(getprop("/surface-positions/flaps-deg") < 1.0) return;

    var current = getprop("/surface-positions/flaps-deg");

    if (step > 0) {
      if (current < 1) {
        interpolate("/surface-positions/flaps-deg", 1, 10 * (1 - current))
      }
    }

    if (step < 0) {
      if (current > 0) {
        interpolate("/surface-positions/flaps-deg", 0, 10 * current)
      }
    }
    if (step == 0) {
        interpolate("/surface-positions/flaps-deg")
    }
};
